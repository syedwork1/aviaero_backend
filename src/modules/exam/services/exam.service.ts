...
export class ExamService {
  ...
  async create(createExamDto: CreateExamDto,userId:string) {
   ...
   const getRandomQuestions= await this.questionSerice.findRandomQuestions(createExamDto.CBR_chapter,createExamDto.difficulty,createExamDto.number_of_questions)
  ...
  return getRandomQuestions.map(async(q) => 
   await this.examQuestionsService.savedStudentExamQuestions(q.id,savedExam.id)
    );
